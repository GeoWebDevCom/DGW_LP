$(document).ready(function () {
    ...
    $("#pagination").pagination({
        ...
        onPageClick: function (pageNumber, event) {
            $(".mayBeHidden").hide();
            var min = (pageNumber - 1) * 6 + 1;
            var max = pageNumber * 6;
...
function SearchVideo() {
    var key = $.trim($("#searchField").val()).toLowerCase();
    if (key != "") {
        var cleanKey = removeDiacritics(key);
        $(".mayBeHidden").hide();
        $(".mayBeHidden").each(function () {
            var title = removeDiacritics($(this).data("title").toString().toLowerCase());
            var author = removeDiacritics($(this).data("author").toString().toLowerCase());
            if (title.indexOf(key) > -1 || author.indexOf(key) > -1) {
                $(this).show();
            }
        });
        $("#pagination").pagination({
            items: $('.mayBeHidden:visible').length,
            itemsOnPage: 6,
            ellipsePageSet: false,
            cssStyle: 'light-theme',
            nextText: "",
